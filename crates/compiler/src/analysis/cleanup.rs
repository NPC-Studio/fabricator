...
pub fn clean_instructions<S>(ir: &mut ir::Function<S>) {
    ...
    ir.instructions
        .retain(|inst_id, _| used_instructions.contains(inst_id.index() as usize));
}

/// Remove all variables that are never used in any block.
///
...
/// Remove all shadow variables that are never used in any block.
///
/// Checks all blocks, does not consider block reachability.
...
/// Remove all this scopes that are never used in any block.
///
/// Checks all blocks, does not consider block reachability.
...
/// Remove all call scopes that are never used in any block.
///
/// Checks all blocks, does not consider block reachability.
pub fn clean_unused_call_scopes<S>(ir: &mut ir::Function<S>) {
    ...
    for block in ir.blocks.values() {
        for &inst_id in &block.instructions {
            let inst = &ir.instructions[inst_id];
            match inst.kind {
                ir::InstructionKind::OpenCall { scope, .. }
                | ir::InstructionKind::FixedReturn(scope, _)
...
/// Remove all functions that are never used in any block.
///
/// Checks all blocks, does not consider block reachability.
