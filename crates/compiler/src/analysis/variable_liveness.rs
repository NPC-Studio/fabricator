...
impl VariableLiveness {
    ...
    pub fn compute<S>(ir: &ir::Function<S>) -> Result<Self, VariableVerificationError> {
            ...
            for (inst_index, &inst_id) in block.instructions.iter().enumerate() {
                match ir.instructions[inst_id] {
                    ...
                    ir::Instruction::CloseVariable(var_id) => {
                        ...
                        {
                            return Err(VariableVerificationError {
                                kind: VariableVerificationErrorKind::MultipleClose,
                                var_id,
                            });
                        }
                    }
        ...
        for &var_id in &variables {
            match ir.variables[var_id] {
                ...
                ir::Variable::Static(_) | ir::Variable::Upper(_) => {
                    if variable_open.contains_key(&var_id) || variable_close.contains_key(&var_id) {
                        return Err(VariableVerificationError {
                            kind: VariableVerificationErrorKind::OpenCloseUnOwned,
                            var_id,
                        });
                    }
            ...
            let &variable_open = variable_open
                .get(&var_id)
                .ok_or(VariableVerificationError {
                    kind: VariableVerificationErrorKind::BadOpen,
                    var_id,
                })?;
            ...
            let variable_liveness =
                ScopeLiveness::compute(ir, &dominators, variable_open, variable_close).map_err(
                    |e| VariableVerificationError {
                        kind: match e {
                            ScopeLivenessError::CloseNotDominated => {
                                VariableVerificationErrorKind::CloseNotDominated
                            }
                            ScopeLivenessError::IndeterminateState => {
                                VariableVerificationErrorKind::IndeterminateState
                            }
                        },
                        var_id,
                    },
            ...
            for &(block_id, inst_index) in &variable_uses[&var_id] {
                let live_range = variable_liveness.for_block(block_id).ok_or({
                    VariableVerificationError {
                        kind: VariableVerificationErrorKind::UseNotInRange,
                        var_id,
                    }
                })?;
